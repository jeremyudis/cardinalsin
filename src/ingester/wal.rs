...
impl WriteAheadLog {
    ...
    pub fn open(config: WalConfig) -> Result<Self> {
        std::fs::create_dir_all(&config.wal_dir).map_err(map_io_error)?;
        ...
        let current_size = std::fs::metadata(&segment_path)
            .map(|meta| meta.len())
            .unwrap_or(0);
        let file = open_segment(&segment_path)?;
    ...
    pub fn truncate_before(&self, seq: u64) -> Result<()> {
        ...
        for segment in segments {
            ...
            if let Some(last_seq) = last_sequence_for_segment(&segment.path)? {
                if last_seq < seq {
                    std::fs::remove_file(&segment.path).map_err(map_io_error)?;
                } else {
                    break;
                }
            }
    ...
    fn rotate(&mut self) -> Result<()> {
        ...
        self.file = open_segment(&self.current_path)?;
        self.current_size = 0;
        Ok(())
    }
...
fn map_io_error(error: io::Error) -> Error {
    if matches!(error.raw_os_error(), Some(libc::ENOSPC)) {
...
mod tests {
    ...
    async fn test_crc_roundtrip() {
        ...
        let mut wal = WriteAheadLog::open(make_config(&dir, 1024 * 1024)).unwrap();
    ...
    async fn test_rotation() {
        ...
        let mut wal = WriteAheadLog::open(make_config(&dir, entry_size + 1)).unwrap();
    ...
    async fn test_truncation() {
        ...
        wal.truncate_before(second_seq).unwrap();
