...
async fn test_complete_query_pipeline() {
    ...
    // Step 3: Execute query through QueryNode::query (not engine bypass).
    ...
    let cpu_rows: usize = batches.iter().map(|batch| batch.num_rows()).sum();
    assert_eq!(cpu_rows, 10, "Should return 10 CPU rows");
}
